...
GaussianPyramid::GaussianPyramid(Image src) {
	setBaseLevel(src);
 ...
 */
void GaussianPyramid::setBaseLevel(const Image& img) {
	/*
	 * Determine if the image can be processed
	 */
	if ((img.NR / pow(2, NUM_LEVELS)) > DIM_LIMIT &&
			(img.NC / pow(2, NUM_LEVELS)) > DIM_LIMIT) {

		for (int i = 0; i < NUM_LEVELS; i++) {
			int rows = (int) ceil (img.NR / pow(2, i));
			int cols = (int) ceil (img.NC / pow(2, i));
...
void GaussianPyramid::reduceImageToLevel(const int srcLevel,
	 ...
	 */
	if (tgtLevel > srcLevel && levels[srcLevel].NC > DIM_LIMIT
			&& levels[srcLevel].NR > DIM_LIMIT) {
			...
			for (int j = 0; j < tgtCols; j++) {
				...
				for (int m = -2; m <= 2; m++) {
					for (int n = -2; n <= 2; n++) {
						...
						if (levels[srcLevel].inboundp(row, col)) {
